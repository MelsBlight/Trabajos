...
const addFormPersona = () => {
    ...
    if (filterDniOne(dni.value, dataPersona) == -1) {
        ...
        dni.focus();
        storageListPerson();
        messageAlert("Datos guardados", "Perfecto");
    }
    else {
        messageAlert("Datos repetidos", "Error")
        dni.focus();
    }
...
const listPersona = (arrayData) => {
    ...
    if (arrayData.length > 0) {
        ...
        list = `<tr><td colspan="6" class="text-center">No se encuentra registro</td></tr>`;
    }
    return list;
}
...
const messageAlert = (menssage, icon = "Perfecto") => {
    Swal.fire({
        position: 'top-end',
        icon: icon,
        title: menssage,
